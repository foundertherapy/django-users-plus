...
if 'debug_toolbar.middleware.DebugToolbarMiddleware' in \
        MIDDLEWARE_CLASSES_NO_DEBUG_TOOLBAR:
    MIDDLEWARE_CLASSES_NO_DEBUG_TOOLBAR.remove(
...
class MasqueradeStartTestCase(django.test.TestCase):
    ...
    def test_user_masquerade_admin_user(self):
        ...
        self.assertEqual(
            c.session['return_page'], 'admin:accounts_user_changelist')
    ...
    def test_user_masquerade_staff_user_no_perm(self):
        c = django.test.client.Client()
        self.assertTrue(
            c.login(email='staffuser@example.com', password='password'))
        r = c.get('/admin/masquerade/3/')
        self.assertRedirects(
            r, '/admin/accounts/user/', fetch_redirect_response=False)
        self.assertIsNone(c.session.get('is_masquerading'))
        self.assertIsNone(c.session.get('masquerade_user_id'))
        self.assertIsNone(c.session.get('return_page'))
        self.assertIsNone(c.session.get('masquerade_is_superuser'))
...
class PasswordResetActionTestCase(django.test.TestCase):
    ...
    def setUp(self):
        group_change_user = django.contrib.auth.models.Group(
            name='Change User')
        group_change_user.save()
        # make sure masquerade group has masquerade permission
        change_user_permission = django.contrib.auth.models.Permission.objects.\
            get(codename='change_user')
    ...
    def test_password_reset_action_admin_user(self):
        ...
        self.assertTrue(
            c.login(email='superuser@example.com', password='password'))
        r = c.post('/admin/accounts/user/', data={
            'action': 'reset_passwords',
            '_selected_action': ['3', '2', ],
        })
        self.assertRedirects(r, '/admin/accounts/user/')
        ...
        self.assertEqual(
            django.core.mail.outbox[0].subject, 'Password reset on example.com')
        self.assertEqual(
            django.core.mail.outbox[1].subject, 'Password reset on example.com')
